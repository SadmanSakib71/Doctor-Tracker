...
export default function PatientsOverTimeChart({ data }) {
  const rows = Array.isArray(data) ? data : [];
  const isEmpty = rows.length === 0 || rows.every((row) => row.count === 0);

  return (
    <section className="min-w-0 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <h3 className="text-sm font-semibold text-slate-900">Patients over Time</h3>
      <p className="mt-1 text-sm text-slate-500">Patients added in the last 30 days</p>

      {isEmpty ? (
        <p className="mt-16 mb-10 text-center text-sm text-slate-500">
          No patients added in the last 30 days.
        </p>
      ) : (
